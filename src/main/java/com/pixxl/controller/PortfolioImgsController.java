...
public class PortfolioImgsController {
    private final String uploadDir =
            System.getProperty("user.dir") + "/uploads/portfolio/";

    @Autowired private PortfolioImgsService portfolioImgsService;

    @PostMapping("/{id}/upload")
    public ResponseEntity<List<Portfolio_imgs>> uploadImagens(
            @PathVariable Long id, @RequestParam("imagem") MultipartFile[] imagens)
            throws IOException {
        List<Portfolio_imgs> imagensSalvas =
                portfolioImgsService.salvarVariasImagensNoPortfolio(id, imagens);
    ...
    public ResponseEntity<List<Portfolio_imgs>> findByPortfolioId(
            @PathVariable Long portfolioId) {
        List<Portfolio_imgs> imagens =
                portfolioImgsService.findByPortfolioId(portfolioId);
        return ResponseEntity.ok(imagens);
    }

    @GetMapping("/por-portfolio/{id}")
    public ResponseEntity<List<Portfolio_imgs>> getImagensPorPortfolio(
    ...
    public ResponseEntity<List<Portfolio_imgs>> findImagensByArtistaId(
            @PathVariable Long artistaId) {
        List<Portfolio_imgs> imagens =
                portfolioImgsService.findImagensByArtistaId(artistaId);
        return ResponseEntity.ok(imagens);
    }

    @PostMapping
    public ResponseEntity<Portfolio_imgs> gravarPortfolioImg(
            @RequestBody Portfolio_imgs portfolio_imgs) {
        Portfolio_imgs salvo =
                portfolioImgsService.gravarPortfolioImgs(portfolio_imgs);
    ...
    @PutMapping("/{id}")
    public ResponseEntity<Portfolio_imgs> update(
            @PathVariable Long id, @RequestBody Portfolio_imgs portfolio_imgs) {
